...
def save_modification(image_seg : napari.layers.Labels, image_raw : ImageData, napari_viewer : Viewer):
    ...
    sousdossier = image_seg.name.split('mask_')[1]
    os.remove(f'{zip_dir.name}\{sousdossier}\{image_seg.name}.png')
    classefleur=np.where(data_label==1)
    data_label[classefleur]=255
    imsave(f'{zip_dir.name}\{sousdossier}\{image_seg.name}.png',data_label)
